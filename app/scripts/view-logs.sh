...
# 小狗星谷服务器 - 日志查看器
#
# Usage: docker exec -it puppy-stardew ${PS_HOME}/scripts/view-logs.sh [option]
# 使用方法：docker exec -it puppy-stardew ${PS_HOME}/scripts/view-logs.sh [选项]
...
# Log directories
CATEGORIZED_DIR="${PS_HOME}/.local/share/puppy-stardew/logs/categorized"
ARCHIVE_DIR="${PS_HOME}/.local/share/puppy-stardew/logs/archive"
...
show_menu() {
    ...
    case $option in
        ...
        7)
            ...
            tail -f ${PS_HOME}/.config/StardewValley/ErrorLogs/SMAPI-latest.txt 2>/dev/null || echo "Log file not found"
            ;;
        0)
            echo "Goodbye!"
            exit 0
...
if [ "$(whoami)" != "steam" ]; then
    ...
    echo "Use: docker exec -it puppy-stardew ${PS_HOME}/scripts/view-logs.sh"
fi
