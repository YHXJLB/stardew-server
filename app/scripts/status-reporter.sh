...
STATUS_FILE="${PS_HOME}/.local/share/puppy-stardew/status.json"
METRICS_FILE="${PS_HOME}/.local/share/puppy-stardew/metrics.prom"
SMAPI_LOG="${PS_HOME}/.config/StardewValley/ErrorLogs/SMAPI-latest.txt"
METRICS_PORT=${METRICS_PORT:-9090}
METRICS_BIND=${METRICS_BIND:-127.0.0.1}   # 仅绑定内网，外部不可达
UPDATE_INTERVAL=15
...
serve_metrics() {
    ...
    while true; do
        ...
        # -q 1: quit 1 second after EOF, -w 5: timeout 5s
        {
            ...
            echo -e "\r"
            echo -n "$body"
        } | nc -l -s "$METRICS_BIND" "$METRICS_PORT" -q 1 -w 5 >/dev/null 2>&1
    done
}
