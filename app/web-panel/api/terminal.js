...
function handleInput(ws, data) {
  ...
  try {
    // 原生 / 非 Docker 部署：优先通过 tmux 会话注入（与 ./pss guard 同机制），
    // 这样即使 SMAPI 的标准输入是 tmux 伪终端也能正确送达。
    const text = data.replace(/\r?\n$/, '');
    let sent = false;
    try {
      const has = spawnSync('tmux', ['has-session', '-t', 'pss'], { stdio: 'ignore' });
      if (has.status === 0) {
        spawnSync('tmux', ['send-keys', '-t', 'pss', text, 'Enter'], { stdio: 'ignore' });
        sent = true;
      }
    } catch (e) { /* tmux 不可用时回退 */ }

    // 回退：Docker 风格的 /proc/PID/fd/0 写法
    if (!sent) {
